...
void playStatusSound(SystemState state)
{
  ...
  case CLEAR:
    for (int i = 0; i < 4; i++)
    {
      tone(BUZZ_PIN, 1600 + (i * 100), 30);
      delay(50);
    }
    break;
  case WALK_IN:
    for (int i = 900; i < 1300; i += 80)
    {
      tone(BUZZ_PIN, i, 15);
      delay(15);
    }
  ...
  case WALK_OUT:
    ...
    break;
  case LOITER:
    tone(BUZZ_PIN, 880, 50);
    delay(100);
    tone(BUZZ_PIN, 880, 50);
    break;
  case LINGER:
    tone(BUZZ_PIN, 1100, 20);
    delay(40);
    tone(BUZZ_PIN, 1100, 20);
    break;
  }
...
void setup()
{
  ...
  nodeMCU.begin(9600); // Starts communication with ESP on Pin 11
  ...
  lcd.createChar(1, charArrowUp);
  lcd.createChar(2, charArrowDn);

  lcd.setCursor(0, 0);
  lcd.print("SEC-CORE v8.0");
  lcd.setCursor(0, 1);
...
void updateLCD()
{
  ...
  case LOITER:
    lcd.print("STATIC+");
    lcd.print(loiterRepetitionCount);
    lcd.print("  ");
    break;
  }
  ...
  lcd.setCursor(0, 1);
  lcd.print("RNG:");
  if (lastValidDist < 100)
    lcd.print(" ");
  if (lastValidDist < 10)
    lcd.print(" ");
  lcd.print(lastValidDist);
  lcd.print("cm ");
  ...
  lcd.setCursor(15, 1);
  if (abs(lastValidDist - lastChangeDist) > MOVE_MARGIN)
    lcd.write(0);
  else
    lcd.print(" ");
}
...
void loop()
{
  ...
  // Send data to ESP8266 via SoftwareSerial (nodeMCU) every 5 seconds
  ...
  if (millis() - lastSendTime > 5000)
  {
    ...
    json += "\"state\":\"" + stateToString(currentState) + "\",";
    json += "\"distance\":" + String(lastValidDist) + ",";
    json += "\"pir\":" + String(debouncedPIR ? 1 : 0) + ",";
    json += "\"presence\":" + String(presence ? 1 : 0) + ",";
    json += "\"last_change_time\":" + String(lastChangeTime) + ",";
    json += "\"last_change_dist\":" + String(lastChangeDist) + ",";
    json += "\"trend_buffer\":" + String(trendBuffer) + ",";
    json += "\"loiter_count\":" + String(loiterRepetitionCount) + ",";
    json += "\"last_pir_trigger\":" + String(lastPirTrigger) + ",";
    json += "\"lcd_on\":" + String(isLCDOn ? 1 : 0);
    json += "}";

    nodeMCU.println("BEGIN");
    delay(10);
    nodeMCU.println(json);
    delay(10);
    nodeMCU.println("END");

    Serial.println("Sent JSON to ESP:");
    Serial.println(json);
  }
